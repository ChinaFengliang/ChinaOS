...
THREAD     *OS_ActiveProc[33];                                              /* 激活的线程队列                       */
THREAD     *OS_This;                                                        /* 当前运行的线程                       */
THREAD     *OS_LiveList;                                                    /* 激活线程链表                         */

static      THREAD      *OS_DeadList;                                       /* 僵尸线程链表                         */
static      IDLE_PCL     OS_Idle;                                           /* 空闲线程控制表                       */
...
INT32S sleep(INT32U Time)
{
    ...
    */
    ...
    {   /* 非唯一节点 */
        ...
        OS_ActiveProc[Priority] = OS_This->pNext;                           /* 更新线程组指针                       */
    }
...
** Descriptions:            检查是否需要切换线程
** Input parameters:
** Output parameters:
** Returned value:          ==0 : 不需要切换
**                          ==1 : 需要切换
**--------------------------------------------------------------------------------------------------------------------
** Created by:              Fengliang
...
{
    /*
     * 如果返回当前线程则不需要切换,否则进行切换.
     *
     * 问: 怎样判断中断服务后是否应该切换?
     * 解: OS_This == 调度算法推选的线程控制块.
     */
...
void startup(void)
{
     ...
     * 1) 复位管理信息
     */
    OS_ActivePrio = 0;                                                      /* 清空事件就绪表                       */
    OS_DeadList   = NULL;
    memset(OS_ActiveProc, NULL, sizeof(OS_ActiveProc));                     /* 清零控制块内容                       */
